...
public class InputCodeActivity extends AppCompatActivity {
    ...
    @BindView(R.id.edt_manual_input_imei)
    ...
    private void init() {
        img_adddv.setVisibility(View.INVISIBLE);
        tx_bar.setText("扫一扫输入");
        hintKbTwo();
        Intent intent=getIntent();
        String code = intent.getStringExtra("json");
        Log.i("init","code            "+code);
...
////                      setResult(REQUEST_CODE, intent);
                    ...
                    String code_edt = edtManualInput.getText().toString().trim();
//                code="867967023179486";
                    if (name.equals("")){
                        Toast.makeText(InputCodeActivity.this, "请输入设备名", Toast.LENGTH_SHORT).show();
                    }else {
                        if (code_edt.equals("")) {
                            Toast.makeText(InputCodeActivity.this, "请输入设备标识码", Toast.LENGTH_SHORT).show();
                        } else {
    ...
    private Handler Datahandler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            ...
            try {
                ...
                Toast.makeText(InputCodeActivity.this, "请输入正确的MAC或SIM卡号或设备esn号", Toast.LENGTH_SHORT).show();
                e.printStackTrace();
                return;
            }
            String name = edtManualInput_name.getText().toString().trim();
